...
=head1 SYNOPSIS
    ...
    $file = NCopy->new(
            ...
            'follow_links'   => 0,
            'force_write'   => 0,
            'set_permission' => \&NCopy::u_chmod,
            'file_check'     => \&NCopy::f_check,
            'set_times'      => \&NCopy::s_times,
    );
    ...
    systems.
    Default for this is \&NCopy::f_check.  On Unix you can also use
    \&NCopy::unix_check.  This one compares the inode and device
    numbers.
    ...
    $file = NCopy->new(recursive => 1);
    $file->copy "file","other_file";
    $file->copy "directory1","directory2";

    $file = NCopy->new(u_chmod => \&my_chmod,f_check => \&my_fcheck);
    $file->copy "directory1","directory2";


=head1 DESCRIPTION

B<NCopy::copy> copies files to directories, or a single file to
...
# $copy = NCopy->new('file_check' => \&NCopy::unix_check);
sub unix_check($$)
{
    my ($file_from,$file_to) = @_;
