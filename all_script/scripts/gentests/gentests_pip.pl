...
my $path2;
if ($^O =~ /win/i) {
    $path = "G:\\xtools\\svauto\\bin\\gentests\\blib\\nt";
    $path2 = "G:\\xtools\\svauto\\bin\\gentests\\";
} elsif ($^O =~ /sol/i) {
    $path = "/proj/xtools/svauto/bin/gentests/blib/sol";
    $path2 = "/proj/xtools/svauto/bin/gentests/";
} else {
    $path = "/proj/xtools/svauto/bin/gentests/blib/lin";
    $path2 = "/proj/xtools/svauto/bin/gentests/";
}
unshift @INC, $path;
unshift @INC, $path2;
...
# Path to sandbox. Used to provide alternative path for files to copy specefy
# in TPR. Necessary for creating regression tests.
local $SANDBOX;
