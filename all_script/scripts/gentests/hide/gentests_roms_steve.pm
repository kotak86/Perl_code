...
#
# Copied from:
# /proj/xtools/roms/ipa5_roms/users/att/roms/ulibs/gentests.pm
...
BEGIN
{
    print "load gentests_steve module...\n";
}
...
#-----------------------------------------------------------------------------
{
    my ($tpr_file, $results_dir, $orig_testcase_dir) = @_;    # $results_dir, and $orig_testcase_dir are new args. SB.
    my $RET_CODE = 7;
    ...
    $main::DEBUG && print "gentests::generate_tpr(): tpr_file = $tpr_file, CURRENT_TPR_FILE = $CURRENT_TPR_FILE\n";
    ...
    # If regressions being run, $TPRFILEDIR will be blank. 
    # Need it to point to original testcase area in order to copy over testbench and data files
    # Would ideally like to access these files from sandbox area but this is more difficult
    # due to different naming conventions for cores in sandbox and results area.
    # (Core names for results area are available to this script but not sandbox core names)
    if($regression)
    {
       $main::DEBUG && print "Modifying TPRFILEDIR.\n";
       $TPRFILEDIR = $orig_testcase_dir . "/common_files/";
    }
	...
	$LINE_NO++;
	chomp();
	ReadTPRLine($_, $regression, $results_dir, $orig_testcase_dir);
    }
    close TPR;


    # copy all the data files to a common directory
    write_common_data_files($orig_testcase_dir);
...
##DOC
#
# sub ReadTPRLine(<line>, $regression)
#
...
sub ReadTPRLine
{
     my ($line, $regression, $results_dir, $orig_testcase_dir) = @_;
     my $common = 0;

     $main::DEBUG && print "gentests::ReadTPRLine(): COMMON_DATA_DIR = $COMMON_DATA_DIR\n";
...
#	 my $env_name = uc($1);
     ...
     elsif ($line =~ /^MODULE_NAME=(.*)$/i) 
     {
         ...
         else  # special case if we are doing regesssion, don't want to get the module name from regnerate.in, get it from testcase results path instead. SB
         {
            $results_dir =~ m/^(\/[\.\w]+){5}\/(\w+)/; # Match 5 levels of hierarchy in the path. 6th will be the core (TPR) name. SB.
	    $MODULENAME = $2;
	    $main::DEBUG && print "gentests::ReadTPRLine: MODULENAME = $MODULENAME\n";
         }
     }
     ...
     elsif  ($line =~ /^COPYDIRS=(.+)$/i)
     {
       $temp = $1;
       $temp =~ s/(\s+)//g;
       @COPYDIRS = InterpretFilenames($temp, $orig_testcase_dir);
	...
	$IPFLOW = cgdata_steve::getIPflow(lc $whattodo);
...
{
     $main::DEBUG && print "gentests::InterpretFilenames(): $_[0], $_[1]\n";
     my @files = split(/\s*,\s*/, $_[0]);
     my @expanded_files;
     my $orig_testcase_dir = $_[1];
     ...
     foreach my $file (@files)
     {
          $main::DEBUG && print "gentests::InterpretFilenames(): file = $file.\n";
...
#-----------------------------------------------------------------------------
{
    ...
    print "\n\nERROR ";
    $main::DEBUG && $calling_sub && print "in gentests_steve::$calling_sub\t";
...
sub write_test
{
    ...
    $main::DEBUG && print "gentests::write_test: COMMON_DATA_DIR = $COMMON_DATA_DIR\n";
   
    # set the family if defined
    if (my $tmp = get_flow_option("FAMILY"))
    {
	$family = $tmp;
	print "In write_test: flow option FAMILY defined. Family = $family\n"; # Steve
    }
    $main::DEBUG && print "gentests::write_test: family = $family\n"; # steve
    ...
    $main::DEBUG && print "gentests::write_test: family_dir = $family_dir\n"; # steve
    ...
    $main::DEBUG && print "gentests::write_test: TPRFILEDIR = $TPRFILEDIR\n";
    $main::DEBUG && system("pwd");

    # Current dir. should be results area for current module.
    if (-e "./ipint.cfg")
    {
	copy("./ipint.cfg", "$datadir/ipint.cfg") or err('--', "Could not create $datadir/ipint.cfg", 2);
    }
...
#-----------------------------------------------------------------------------
{
    ...
    print FILE "whattodo = ", cgdata_steve::getWhattodo($IPFLOW), "\n";
...
#-----------------------------------------------------------------------------
{
    ...
    my $cfg_exists = 0;
    my %settings;
    $main::DEBUG && print "gentests::write_cfg_file: TPRFILEDIR = $TPRFILEDIR\n";

    # Current dir. should be results area for current module.
    if (-e "./gentests.cfg")
    {
	$cfg_exists = 1;
	%settings = ipint_final_cfg::read_cfg("./gentests.cfg");
...
#-----------------------------------------------------------------------------
{
    my $orig_testcase_dir = $_;
    my @data_files;
    my $ncopy = NCopy->new('force_write' => 1);
    my $ncopy_recursive = NCopy->new('force_write' => 1, 'recursive' => 1);

    $main::DEBUG && print "gentests::write_common_data_files: TPRFILEDIR = $TPRFILEDIR\n";
    $main::DEBUG && print "gentests::write_common_data_files: orig_testcase_dir = $orig_testcase_dir\n";
    $main::DEBUG && print "gentests::write_common_data_files: COMMON_DATA_DIR = $COMMON_DATA_DIR\n";

    if (($GLOBAL_FLOWOPTS{'REGENERATED'} || $main::UNIVERSAL_FLOWOPTS{'REGENERATED'}) && -e $TPRFILEDIR."/gentests.cfg")
    {
	my %settings = ipint_final_cfg::read_cfg($TPRFILEDIR."/gentests.cfg");

	if (exists $settings{'ARRAY'}{'VHD_TBs'})
	{
	    foreach my $file (@{$settings{'ARRAY'}{'VHD_TBs'}})
	    {
	       my $full_file_path = $orig_testcase_dir . "/common_files/" . $file;
               $main::DEBUG && print "gentests::write_common_data_files: orig_testcase_dir = $orig_testcase_dir, file = $file\n";
	       $main::DEBUG && print "gentests::write_common_data_files: full_file_path = $full_file_path\n";

		# use the _orig tb if it exists
 		if (-e $full_file_path."_orig")
 		{
		    $ncopy->copy($full_file_path."_orig", $COMMON_DATA_DIR."/".$file) or err('--', "Could not copy the file $full_file_path to $COMMON_DATA_DIR", 2);
 		}
	    }
	}
    ...
    {
       my $full_file_path = $orig_testcase_dir . "/common_files/" . $file;
       $main::DEBUG && print "gentests::write_common_data_files: file in array data_files = $file\n";
       $main::DEBUG && print "gentests::write_common_data_files: full_file_path = $full_file_path\n";

	# only copy non variable files
	if ($file !~ /\$/)
	{
           $main::DEBUG && print "gentests::write_common_data_files: CWD:";
           $main::DEBUG && system("pwd");
	   $ncopy->copy($full_file_path, $COMMON_DATA_DIR."/".$file) or err('--', "Could not copy the file $full_file_path to $COMMON_DATA_DIR", 2);
	}
    }
	...
	if (-d $dir)
	{
           $ncopy_recursive->copy($dir, $COMMON_DATA_DIR."/") or err ('--', "Could not copy the dir $dir to $COMMON_DATA_DIR", 2);
        }	
	else
	{
	    # remove the file portion and work
		...
		$target_dir = $COMMON_DATA_DIR."/".$target_dir;
            ...
            $main::DEBUG && print "gentests::write_common_data_files: target_dir = $target_dir\n";
...
#-----------------------------------------------------------------------------
{
	...
	{
	    # CR 198584 fix
	    print( "gentests_steve.pl locked.  Sleeping (", scalar(localtime), ")\n" ), $poll=1 unless $poll;
	    sleep 5;
	    $wait_period += 5 ;
        ...
        print( "gentests_steve.pl lock released (", scalar(localtime), ")\n" ), if $poll;
...
# If generating regression tests, Final.cfg will be copied to new (regression) testcase dir from
# results area. However, it will contain parameter settings which are incorrect for regression tests.
...
#-----------------------------------------------------------------------------
{
   ...
   $main::DEBUG && print "gentests::modify_final_cfg: regression_testlevel = $regression_testlevel\n";
	...
	if ($key eq "REPOSITORY_WIN") 
	{
	   ...
	   print "Modified REPOSITORY_WIN\n"; 
	}
	elsif ($key eq "REPOSITORY_UNIX")
	{
	   $value = $repos;
	   print "Modified REPOSITORY_UNIX\n";
	}
	elsif ($key eq "TESTLEVEL")
	{
	   $value = $regression_testlevel;
	   print "Modified TESTLEVEL\n";
	}
	elsif ($key eq "VSIM_TIMELIMIT")
	{
	   $value = $regressions_vsim_timelimit;
	   print "Modified VSIM_TIMELIMIT to $regressions_vsim_timelimit minutes.\n";
	}	

        # Don't want any of the following params written to the new Final.cfg.
	# They are platform-dependent paths. Let the run script or easytest decide what platform to run on.
	# Also, CADSET_xilinx will be defined by the run flow.
	elsif ( ($key eq "CADSET_xilinx") or ($key eq "perl") or ($key eq "REPOSITORY") or ($key eq "REPOSITORYROOT") or ($key eq "RESULTSROOT") or ($key eq "SPOOLER") or ($key eq "XCOMPLIBS") or ($key eq "tc") )
	{
	   $skip = 1;
	   print "Set skip because key = $key\n";
	}	
   ...
   {
      $main::DEBUG && print "gentests::modify_final_cfg: $key $settings{$key} after skipping\n";
      print TEMP "$key $settings{$key}\n";
   }
   close TEMP;

   my $command = "mv $cfg_file $cfg_file" . ".original";
   $main::DEBUG && print "gentests::modify_final_cfg: command = $command\n";
