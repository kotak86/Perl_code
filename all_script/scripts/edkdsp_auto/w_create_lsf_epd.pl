...
if ($os =~ /lin/) {
 ...
 if ($sim =~ /isim/) {
   ...
   if (! -d "/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk") {
		 $cp = `mkdir -p /proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk`;
	 }
  	...
  	foreach $arch (@arch)
  	{
  	   if (($arch =~ /^\./) || ($arch =~ /common_files/) || ($arch =~ /xml/) || ($arch =~ /save/) )
  	   {
          ...
          if (-e "$dir/$core/$arch/$case") {
  	         open (lsf1,">>/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os");
		...
		open (FILE,"/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os");
		while (<FILE>) 
		{
		  $line = $_;
		...
		$rm_tmp = `rm -rf /proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os`;
		
		open (lsf,">/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_$os");
		  print lsf "#!/bin/csh\n";
		  ...
		  print lsf ");\n";
		  print lsf "/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_$wrapper \$a\[\$LSB_JOBINDEX\];\n";
		  close(lsf);
  }
}
...
if ($os =~ /lin/) {
	open (wrap,">/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_$wrapper");
	...
	#		$ts = "/proj/xtools/svauto2/flows/xlnxIP/prod/xlnxIP.pl -tc \$argv\[\*\] -CADMAN_xilinx $REL -CADMAN_xilinx_pathtype $xlnx_path $sim_vendor -SIMULATOR modelsim -SUPER_TESTSUITE EDK -TESTSUITE IP_2012.2_12$bld  -RESULTSROOT /proj/xresults/arizona/$ENV{USER} -ENV_XIL_PAR_SS_DESIGN 1 -LSF_OPTION \' -P ip_ipv_regressions -q medium -R \"rusage\[mem=3000\]\" -R \"rusage\[swesvverification=2:swemsimhdlsim=2:swemsimhdlmix=2\]\" -oo /proj/xtools/testRunner/logs/$ENV{USER}/\%J_\%I.txt -eo /proj/xtools/testRunner/logs/$ENV{USER}/\%J_\%I.err -E /proj/xtools/cfs/lsf_validate.sh -Lp ipreg_opt \' -XIL32 0 -DEBUGLEVEL 1 -DEBUG_MODE 0 -RODIN_FLOW 1 -SIMULATION none -PLATGEN_OPTION \'-intstyle vivado\' ";

	 $ts = "/proj/xtools/svauto2/flows/xlnxIP/prod/xlnxIP.pl -tc \$argv\[\*\] -CADMAN_xilinx $REL -CADMAN_xilinx_pathtype $xlnx_path $sim_vendor -SIMULATOR modelsim -RESULTSROOT /proj/xresults/arizona/$ENV{USER} -ENV_XIL_PAR_SS_DESIGN 1 -LSF_OPTION \' -P ip_ipv_regressions -q medium -R \"rusage\[mem=2000\]\" -R \"rusage\[swesvverification=2:swemsimhdlsim=2:swemsimhdlmix=2\]\" -oo /proj/xtools/testRunner/logs/$ENV{USER}/\%J_\%I.txt -eo /proj/xtools/testRunner/logs/$ENV{USER}/\%J_\%I.err -E /proj/xtools/cfs/lsf_validate.sh -Lp ipreg_opt \' -XIL32 0 -DEBUGLEVEL 1 -DEBUG_MODE 0 -RODIN_FLOW 1 -PLATGEN_OPTION \'-intstyle vivado\' -SIMULATION none";
 ...
 if (!-e "/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$wrapper") {
 	open (wrap,">/proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$wrapper");
    print wrap "\@echo off\n";
...
$rm_tmp = `rm -rf /proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os`;
  ...
  foreach $tcases_list (@tcases_list) {
		open (lsf,">>/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/bvt_nt_lsf_jobs");
	   if ($hwcosim =~ /1/) {
		     if ($family =~ /virtex5/) {
...
$permission = `chmod -R 755 /proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk`;
print "lsf files for $os created at : /proj/ipco/users/hshah/ankit/lsf/$ENV{USER}_lsf/$REL\_edk\n";
$os = ();
$core = ();
$family = ();
