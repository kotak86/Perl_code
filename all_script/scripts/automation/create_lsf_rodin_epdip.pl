...
{print "Usage create_lsf.pl <file name> <platform lin/nt> <core name> <family> <core version : L or K > <run type : BVT or REGR> <simulator : isim, mti, ius> <site: XSJ, XCO> \n";exit;}
 ...
 if ($REL =~ /^ids/){
        $dir = "/proj/XCOtcRepo/ipv/automation/vivado_full/edkIP/_TCASE/edkIP";
    }
...
$dir = "/proj/XCOtcRepo/ipv/automation/vivado_static/edkIP/_TCASE";

print $dir
}
}

# $dir = "/proj/xtcRepo/ipv/automation/vivado_static/edkIP/$run_type/_TCASE/edkIP";
# $dir_nt = "G:\\xtcRepo\\ipv\\automation\\vivado_static\\edkIP\\$run_type\\_TCASE\\edkIP"; # Make sure the path is visible from all windows machine for LSF job #
...
if ($os =~ /lin/) {
 ...
 if ($sim =~ /isim/) {
   ...
   if (! -d "/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk") {
		 $cp = `mkdir -p /proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk`;
	 }
  	...
  	foreach $arch (@arch)
  	{
  	   if (($arch =~ /^\./) || ($arch =~ /common_files/) || ($arch =~ /xml/) || ($arch =~ /save/) )
  	   {
          ...
          if (-e "$dir/$core/$arch/$case") {
  	         open (lsf1,">>/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os");
		...
		open (FILE,"/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os");
		while (<FILE>) 
		{
		  $line = $_;
		...
		$rm_tmp = `rm -rf /proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os`;
		
		open (lsf,">/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_$os");
		  print lsf "#!/bin/csh\n";
		  ...
		  print lsf ");\n";
		  print lsf "/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_$wrapper \$a\[\$LSB_JOBINDEX\];\n";
		  close(lsf);
  }
}
...
if ($os =~ /lin/) {
	...
	open (wrap,">/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_$wrapper");
	...
	$ts = "/proj/xtools/svauto2/flows/xlnxIP/prod/xlnxIP.pl -tc \$argv\[\*\] -CADMAN_xilinx $REL -CADMAN_xilinx_pathtype $xlnx_path $sim_vendor -SIMULATOR modelsim -SUPER_TESTSUITE EDK $test_suite -RESULTSROOT /proj/xresults/arizona/rodin -ENV_XIL_PAR_SS_DESIGN 1 -LSF_OPTION \' -P ip_ipv_regressions -G ipco_grp -q medium -R \"rusage\[mem=3000\]\" -R \"rusage\[swesvverification=2:swemsimhdlsim=2:swemsimhdlmix=2\]\" -oo ./\%J_\%I.txt -eo ./\%J_\%I.err -Lp ipreg_opt \' -XIL32 0 -DEBUGLEVEL 1 -DEBUG_MODE 0 -RODIN_FLOW 1 -SIMULATION none -PLATGEN_OPTION \'-intstyle vivado\' ";
 ...
 if (!-e "/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$wrapper") {
 	open (wrap,">/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$wrapper");
    print wrap "\@echo off\n";
    ...
    if ($hwcosim =~ /1/) {
       if ($family =~ /virtex5/i) {
	 				$ts = "G:\\xgs\\perl\\5.8.5\\bin\\perl G:\\xtools\\svauto2\\flows\\dspIP\\dev_ipxact\\dspIP.pl -CADMAN_xilinx $REL -CADMAN_xilinx_pathtype $xlnx_path  $sim_vendor $typ -RESULTSROOT_WIN  G:\\xresults\\arizona -SIM_TYPE routed -FAMILY virtex5 -HW_COSIM 1 -HW_BOARD ml506-jtag -ENV_XIL_NETGEN_REMOVE_GTXE1 1 -tc \%\* ";
       }
       elsif ($family =~ /spartan3adsp/i) {
	 				$ts = "G:\\xgs\\perl\\5.8.5\\bin\\perl G:\\xtools\\svauto2\\flows\\dspIP\\dev_ipxact\\dspIP.pl -CADMAN_xilinx $REL -CADMAN_xilinx_pathtype $xlnx_path  $sim_vendor $typ -RESULTSROOT_WIN  G:\\xresults\\arizona -SIM_TYPE routed -FAMILY spartan3adsp -HW_COSIM 1 -HW_BOARD ml506-jtag  -ENV_XIL_NETGEN_REMOVE_GTXE1 1 -tc \%\* ";
       }
    }
    else {
	 				$ts = "G:\\xgs\\perl\\5.8.5\\bin\\perl G:\\xtools\\svauto2\\flows\\dspIP\\dev_ipxact\\dspIP.pl -CADMAN_xilinx $REL -CADMAN_xilinx_pathtype $xlnx_path  $sim_vendor $typ -RESULTSROOT_WIN  G:\\xresults\\arizona -SIM_TYPE routed -ENV_XIL_NETGEN_REMOVE_GTXE1 1 -tc \%\* ";
    }
		print wrap "$ts\n";
		$ts = ();
	close(wrap);
...
$rm_tmp = `rm -rf /proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk/$run_type\_$core\_$family\_lsf_job_try_$os`;
  ...
  foreach $tcases_list (@tcases_list) {
		open (lsf,">>/proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL/bvt_nt_lsf_jobs");
	   if ($hwcosim =~ /1/) {
		     if ($family =~ /virtex5/) {
...
$permission = `chmod -R 755 /proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk`;
print "lsf files for $os created at : /proj/ipco/users/hshah/lsf/$ENV{USER}_lsf/$REL\_edk\n";
$os = ();
$core = ();
$family = ();
