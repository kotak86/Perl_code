...
my $script_dir = "/proj/XCOtcRepo/ipv/automation/scripts"; # Change to your local path #
...
my $crtlsf_dsp = "$script_dir/a_create_lsf_rodin_dspip.pl";
my $crtlsf_edk = "$script_dir/a_create_lsf_epd.pl";
#my $lsf_loc = "/proj/ipco/users/hshah/lsf/hshah_lsf";
...
sub cretlsf_n_jobsubmit {
     my $crt_lsf = $_[0];  my $lsfarg_list = $_[1]; my $ip_type = $_[2];
     my $lsf_dir;
	...
	print "job_submitted & $REL\_submitd file created.\n";
	   ...
	   if($_ eq $REL){
	      ...
	      print "$REL found.\n";
		     ...
		     print "$REL directory & $file found for build at $dte_splt[3].\n";
                     &cretlsf_n_jobsubmit($crtlsf_dsp,$dsparg_list, "dsp");
		     &cretlsf_n_jobsubmit($crtlsf_edk,$edkarg_list, "edk");
		     my $result = "http://xcorvprod/sfprojects/RV/web/view/showexp?Site=CO%3B%3BIN&Build=$REL&OSName=&Family=&Core=&Status=&UserName=hshah&SuperSuite=NEW&TestSuite=&Flow=&Device=&TestcaseName=&ErrorStr=&CRNum=&Notes=&fromdate=&todate=&daterange=&RunAgain=RunAgain&formName=expform&formurl=showexp&table_data=&userId=hshah";
