  ...
  while (<client>)
  {
    ...
    print "Core name is : $word1 , Core version is : $word2  \n" if $DEBUG;
		...
		open (Write, ">>my_p4client");
		  print Write "\/\/IP3\/DEV\/hw\/$word1\/\* \/\/IP3_$ENV{USER}_edkcore_regressions\/DEV\/hw\/$word1\/\* \n";
		  print Write "\/\/IP3\/DEV\/hw\/$word1\/$word2\/... \/\/IP3_$ENV{USER}_edkcore_regressions\/DEV\/hw\/$word1\/$word2\/... \n";
	  close(Write);
  }
close(client);
