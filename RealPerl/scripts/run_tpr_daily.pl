...
while (<FILE>)
{
     ...
     {
       if (-e "$tpr_path/$core/test/$core_ver\_daily.tpr"){
	   $gentests =  `cd $tpr_path/$core/test; /proj/xtools/svauto2/bin/gentests/gentests.pl -d $dir/$ENV{REL}/$run_type $core_ver\_daily.tpr`;
