...
# $Header: /devl/xcs/repo/env/Misc/TestAutomationTools/Arizona/svauto/bin/gentests/Attic/gentests.pm,v 1.1.2.20 2009/04/10 08:45:29 niamhc Exp $
...
#
# IMPORTANT - INSTRUCTIONS FOR UPDATING THIS FILE
#
# By all means update this file to add features you need and/or fix bugs.
#
# However (and this is the "important" bit) bear in mind that this gentest
# is used by several disparate sets of users, who can have significantly
# different requirements  ..most notably..
#      - ACND ip testers+developers
#      - DSP ip testers+developers
#      - EDK ip testers+developers
#      - IP-regression testers in all three areas above
#
# Consequently..
#      - Ensure any modification is 100% backward compatible
#
#      - If you must make a mod which might break other existing features,
#        make it specific to your flow/area  e.g.. wrap it in something
#        like..
#              if( $IPFLOW =~ /^EDK/i ) {  
#                  ...your mods here.. 
#              }
#
#      - Do NOT delete/comment code just because it seems irrelevant to your
#        requirements.
# 
# Pls, do not delete this message
#
##############################################################################
...
# define the files related to a flow flow.
# every main key (whattodo) must always be lower case
my %flow_data = (
		 'characterise' => { 'ipflow' => "char", 'main' => "char.pl" },
		 ...
		 'simulate' => {
				...
				# the main flow file
			        'main' => "dspIP.pl",
				# a list of supported simulators
				'simulators' => ["mti", "mxe", "ncsim", "vcs", "scirocco"]
			       },
		 ...
		 'network' => {
				...
				# a list of supported simulators
			       'simulators' => ["mti"],
				# the order in which the MTI files are run
				# the first file must be the setup file
			       'mti' => ["netIPtest_mti_setup.flw", "netIPtest_mti_sim.flw", "netIPtest_mtidemo_sim.flw"],
			      },
		 ...
		 'network2' => {
				...
				# the main flow file
			       'main' => "netIP2test.flw",
				# any sub flows which are called
   				 'sub_flows' => ["netIP2test_coregen.flw", "netIP2test_implement.flw"],
				# a list of supported simulators
			       'simulators' => ["mti"],
				# the order in which the MTI files are run
				# the first file must be the setup file
			       'mti' => ["netIP2test_mti_setup.flw", "netIP2test_mtidemo_sim.flw"],
			      },

		 'structip' => { 'ipflow' => "structIP", 'main' => "structIP.pl" },

		 'pipsimulate' => {
				   'ipflow' => "PIPtest",
				   'main' => "PIPtest.flw",
				   'sub_flows' => ["PIPtest_coregen_map.flw"],
				   'simulators' => ["mti"],
				   'mti' => ["PIPtest_mti_setup.flw", "PIPtest_mti_sim.flw"],
				  },

		 'edkiptest' => {
				   'ipflow' => "EDKIPtest",
				   'main' => "EDKIPtest.flw",
				   'sub_flows' => ["EDKIPtest_platgen_map.flw", "EDKIPtest_genrefcore.flw"],
				   'simulators' => ["mti", "ncsim"],
				   'mti' => ["EDKIPtest_mti_setup.flw", "EDKIPtest_mti_sim.flw", "EDKIPtest_genrefcore_sim.flw"],
				   'ncsim' => ["EDKIPtest_ncsim_setup.flw", "EDKIPtest_ncsim_sim.flw", "EDKIPtest_genrefcore_sim.flw"],
				  },
		 'edkhwtest' => {
				   'ipflow' => "EDKHWtest",
				   'main' => "ispt.flow",
		 ...
		 'bvt_coregen' => {
				   'ipflow' => "BVT_coregen",
				   'main' => "bvt_coregen.flw",
				  },

		 'check_edif' => {
				  'ipflow' => "coregentest",
				  'main' => "coregentest.flw",
				 },
...
#-----------------------------------------------------------------------------
{
    ...
    my $regression = 0;
    if ($tpr_file =~ m/.*(\.in$)/)                            # If the tpr is "regenerate.in", need to let readTPRline() know
    {
       $main::DEBUG && print "gentests::generate_tpr(): Generating regressions.\nresults_dir = $results_dir\n";
       $regression = 1;
    } 
    $main::DEBUG && print "gentests::generate_tpr(): COMMON_DATA_DIR = $COMMON_DATA_DIR\n";


    # start the timer
    my $start = (times)[0];

    # run script to create testcases if it exists
    # find directory that TPR file is in
    my $tpr_dir;
    print "Setting up tpr directory...\n";
    if ($tpr_file =~ /(\w+)\.tpr/) {
      if ($tpr_file =~ /(.*)$1\.tpr/) {
        $tpr_dir = $1;
        if ($tpr_dir eq "") {
        ...
        print "TPR directory is $tpr_dir\n";
      }
    }
    if (-e "$tpr_dir/create_testcases_randomonly.pl") {
        print "Running $tpr_dir/create_testcases_randomonly.pl script to create random testcases...\n";
        if (-e "random.dat") {
            if (system("chmod 777 random.dat") != 0) {
                die "Couldn't make random.dat editable\n";
            }
        }
        if (system("perl $tpr_dir/create_testcases_randomonly.pl") != 0) {
            die "Couldn't execute perl script\n"; 
        }
    }
    if (-e "$tpr_dir/create_testcases_allranges.pl") {
        print "Running $tpr_dir/create_testcases_allranges.pl script to create ranges testcases...\n";
        if (-e "ranges.dat") {
            if (system("chmod 777 ranges.dat") != 0) {
                die "Couldn't make ranges.dat editable\n";
            }
        }
        if (system("perl $tpr_dir/create_testcases_allranges.pl") != 0) {
            die "Couldn't execute perl script\n"; 
        }
    }
    ...
    #local $COMMON_DATA_DIR = "/products/xirsqa/sandboxes/sandbox_elinden/env/Databases/ip2/processor/hardware/opb_pci/opb_pci_v1_02_a/svg";
...
sub ReadTPRLine
{
    ...
    if ($line =~ /^flowoption\s+(\w+)=(.*)$/i)
    {
	 ...
	 $main::DEBUG && print "TPR Flow Option: $flowopt_name = $flowopt_val # $comment\n";
    }
...
#	 my $env_name = uc($1);
    ...
    elsif  ($line =~ /^COPYDIRS=(.+)$/i)
    {
      ...
      @UNEXPANDED_COPYDIRS = split(/,/,$temp);
	...
	$whattodo = lc($1) ;
	$whattodo =~ s/\s*$//;
        $whattodo = 'characterise' if $whattodo =~ /^\s*char/i ;
        $whattodo = 'structip'     if $whattodo =~ /^\s*structip/i ;
        $whattodo = 'simulate'     if $whattodo =~ /^\s*dspip/i ;

	$IPFLOW = &getIPflow($whattodo);
...
#	    warning ('--', "The coe_input function is depreciated.");
       ...
       elsif ($line =~ /^(float|int|string)?\s*flowoption\s*([^=]+)=(.*)$/i)
       {
	    my $type = lc($1);
	    my $param_names = $2;
	    my $param_vals = $3;
...
#	    $LOCAL_FLOWOPTS_COMMENTS{$flowopt_name} = $comment;
       ...
       elsif ($line =~ /^COPYDIRS=(.+)$/i)
       {
        ...
        @UNEXPANDED_LOCAL_COPYDIRS = split(/,/,$temp);
       ...
       elsif ($line =~ /END/i)
       {
	    ...
	    if (!$main::KEEP_EXISTING_TESTS && !$RM_EXISTING_TESTS_DONE && (($GLOBAL_FLOWOPTS{'REGENERATED'} != 1) && ($main::UNIVERSAL_FLOWOPTS{'REGENERATED'} != 1)))
	    {
...
sub InterpretFilenames
{
     ...
     #
     # NOTE  ..removed here what looked to be flaky code referencing a "$sandbox" 
     # var which is not initialised anywhere ..Reinstated original coge segment from
     # previous file version which simply prepended file paths with $TPRFILEDIR
     # liamb, 1-May-07, in v1.10 of az genests.pm
     if ($^O =~ /^MSWin/)
     {
     ...
     foreach my $file (@files)
     {
	  ...
	  # otherwise see if compressed file exists
	  elsif (-r "$file.gz" )
	  {
               system( "gzip -d $file.gz" ) ;
	       push (@expanded_files, $file) if -r $file ;
               err( $LINE_NO, "Failed to unzip, $file.gz", 1) unless -f $file ;
	  }
	  else
	  {
...
sub CreateTest
{
     ...
     # Make a global ref to the saved data-struct so other subs can inspect it's contents
     # LB 1-Aug-06
     $saved_vht_param = \%SAVED_VHT_PARAM ;
     ...
     if (! $ThisIsARecursiveCase)
     {
	  ...
	  foreach $element (keys %VHT_PARAM)
	  {
	       $main::DEBUG_OUT && print "\n$element-> ";
            ...
            foreach my $flowOpt (@FLOWOPTION_ARRAY) {
                if ($element eq $flowOpt) {
                    $isFlowOpt = 1;
                }
            }
            if ($isFlowOpt == 1) {
                my $flowoptKey = uc($element);
	            $LOCAL_FLOWOPTS{$flowoptKey} = $VHT_PARAM{$element}[0];
                $isFlowOpt = 0;
            }
	       ...
	       foreach my $elem_cnt (0..$#{$VHT_PARAM{$element}})
	       {
		    ...
		    {
			if (($IPFLOW =~ /^EDK\w+test$/i)||($IPFLOW =~ /^edkIP$/i))
			{
...
if ((($value =~ /[\(\)\=\<\>\!\|\&\*\+\-\%\/]+/) || ($value =~ /\{([0-9a-zA-Z_]+)\}ne\{([0-9a-zA-Z_]+)\}/) || ($value =~ /\{([0-9a-zA-Z_]+)\}eq\{([0-9a-zA-Z_]+)\}/)) && $value !~ /^[\+-]\d+$/  && $value !~ /;/)
...
{
    my $tcase_name;
    my $is_regr = 1 if ($GLOBAL_FLOWOPTS{'REGENERATED'} == 1) || ($main::UNIVERSAL_FLOWOPTS{'REGENERATED'} == 1) ;
    ...
    if (($GLOBAL_FLOWOPTS{'REGENERATED'} == 1) || ($main::UNIVERSAL_FLOWOPTS{'REGENERATED'} == 1) || ($main::USE_FILENAME))
    {
	$tcase_name = $FILENAME;
        my $tcase_nmr = 1 ;
        $tcase_nmr = 1 + $dejavu{$tcase_name}  if defined $dejavu{$tcase_name} ;
        $tcase_name = $tcase_name . $tcase_nmr if $tcase_nmr > 1 ;
        chop( $tcase_name ) if $tcase_name =~ /_$/ ;
        $tcase_name = $main::PREFIX_NAME . $tcase_name ;
        $dejavu{$FILENAME} = $tcase_nmr ;
        print "Generating $tcase_name, prefix_name = ", $main::PREFIX_NAME, "\n" ;
    }
    else
    {
    ...
    # Copy and unpack Final.cfg file

    if ($is_regr && -e "./Final.cfg.gz")
    {
	copy("./Final.cfg.gz", "$datadir/Final.cfg.gz") or err('--', "Could not create $datadir/Final.cfg.gz", 2);
	system("/usr/bin/gunzip $datadir/Final.cfg.gz");
        $main::DEBUG && print "gentests::write_test: Found Final.cfg.gz, copied to $datadir and gunzipped.\n";
	modify_final_cfg($datadir, "/Final.cfg");
    }
    elsif ($is_regr && -e "./Final.cfg")
    {
	copy("./Final.cfg", "$datadir/Final.cfg") or err('--', "Could not create $datadir/Final.cfg", 2);
        $main::DEBUG && print "gentests::write_test: Found Final.cfg and copied to $datadir.\n";
	modify_final_cfg($datadir, "/Final.cfg");
    }
	...
	copy ($datafile, $datadir) or err('--', "Could not copy the file $datafile to $datadir.", 2);
	$main::DEBUG && print ("LOCAL_COPYFILES: file $datafile copied to $datadir.\n");
    }
	...
	if (-d $d)
	{
	    NCopy::copy (\1, $d, $datadir) or err('--', "Could not copy the file $d to $datadir", 2);
	    fix_all_permissions($datadir);
	}
	else
	{
    ...
    foreach $interflowoption (@FLOWOPTION_ARRAY)
    {
	delete ($VHT_PARAM{$interflowoption});
    }
    #print "lbdbg - IPFLOW = $IPFLOW\n" ;
    # write out the paramater files depending on flow
    if (($IPFLOW =~  /^EDK\w+test$/i) || ($IPFLOW =~ /^edkIP$/i))
    {
	write_edk($datadir, $MODULE, $CORE_VERSION);
    write_vsim_args($datadir, $MODULE, $CORE_VERSION);
    #write_ncsim_args($datadir, $MODULE, $CORE_VERSION);
    write_sim_args($datadir, $MODULE, $CORE_VERSION);
	write_top_generics($datadir, $MODULE, $CORE_VERSION);
    }
    elsif (($IPFLOW =~ /^edkipsys$/i) || ($IPFLOW =~ /edk_dev/i) || ($IPFLOW =~ /edkip_hwut/i))
    {
	write_mhs($datadir, $MODULE, $CORE_VERSION);
    }
    elsif ( $IPFLOW =~ /^dspip$/i and $is_regr )
    {
        print "is_regr = $is_regr\n" ;
	write_xco($datadir, $tcase_name, $family, $MODULE, $CORE_VERSION);

	# use core.bat from original run to guarantee regression runs in identical fashion
	copy("./core.bat", "$datadir/core.bat") or err('--', "Could not create $datadir/core.bat", 2);
...
#-----------------------------------------------------------------------------
{
    return $main::REPBRANCH if $IPFLOW =~ /dspIP|structIP/ ; # All dsp dev is on HEAD branch
    return $main::REPBRANCH if $IPFLOW eq 'char' ; # All char tests go on HEAD "branch"
...
#-----------------------------------------------------------------------------
{
    ...
    if (@UNEXPANDED_COPYDIRS)
    {
	my $dir;

	print FILE "copydirs = ";

	foreach my $cnt (0..($#UNEXPANDED_COPYDIRS-1))
	{
	    $dir = $UNEXPANDED_COPYDIRS[$cnt];
        #$dir =~ s/(\/|\\)$//;
        #$dir =~ s/^.*(\/|\\)//;
        if ($dir =~ /(\w+)/) {
            $dir = $1;
	...
	$dir = $UNEXPANDED_COPYDIRS[$#UNEXPANDED_COPYDIRS];
    if ($dir =~ /(\w+)/) {
        $dir = $1;
    }
    #$dir =~ s/(\/|\\)$//;
	...
	if (!$GLOBAL_FLOWOPTS{'REGENERATED'} && !$main::UNIVERSAL_FLOWOPTS{'REGENERATED'})
	{
		...
		{
                    # Dont print any "0." values -print "0" instead (CR448128)
                    my $prtDat = $VHT_PARAM{$param}[$elem] ;
                    $prtDat = "0" if sprintf($prtDat) =~ /^[-0\.]+$/ ; 
		    print FILE "$prtDat ";
...
#-----------------------------------------------------------------------------
{
    ...
    print STIMDO ("onerror \{ quit \}\n");
    print STIMDO ("onbreak \{ quit \}\n");
    print STIMDO ("run -all\n");
    print STIMDO ("quit -f\n");
    close STIMDO;

    open (STIMDO,">$dir/stim_ius.do") or err('--', "Cannot create the stim_scirocco.do file in $dir.", 2);
    print STIMDO ("set intovf_severity_level warning\n");
    print STIMDO ("set assert_stop_level failure\n");
    print STIMDO ("run\n");
    print STIMDO ("exit\n");
    close STIMDO;
}
...
#-----------------------------------------------------------------------------
{
    ...
    # Create special family rule if "magic" family_force has been defined -this variable
    # facilitates full randomisation of FAMILY selection, at test **generation** time
    # LB 1-Aug-06
    for my $i (0 .. $#TEST_VARHASH) {
         if ($TEST_VARHASH[$i]{'parameter_name'} eq 'family_force' 
         && defined($GLOBAL_FLOWOPTS{FAMILY_FORCE})  ) {
            print "Restricting test to family, ", $saved_vht_param->{'family_force'}[0], "\n" ;
	    print XCO "#Rule:--FAMILY eq '", $saved_vht_param->{'family_force'}[0], "'--\n" ;
         }
    }
...
#-----------------------------------------------------------------------------
{
    ...
    open(EDK, ">$dir/sim_parameters.args") or err('--', "Could not create sim_parameters.args for writing:$!", 2);
...
#-----------------------------------------------------------------------------
{
    ...
    #only write parameters beginning with C_ (core parameters) to this file
    #TX_CLK_PERIOD RX_CLK_PERIOD special case for opb_ethernetlite
    #DDR_CLK90_PS special case for opb_ddr
    #NUM_ECC_BITS special case for plb_ddr
    #OPB_WIDTH, OPB_BEWIDTH special case for opb_hwicap
    if ($key !~ /^C_/i) {
        if (($key !~ /NUM_ECC_BITS/i) && ($key !~ /OPB_WIDTH/i) && ($key !~ /OPB_BEWIDTH/i) ) {
            next;
        }
    } else {
        if (($key =~ /C_TX_CLK_PERIOD/i) || ($key =~ /C_RX_CLK_PERIOD/i) || ($key =~ /C_DDR_CLK90_PS/i)) {
            next;
        }
    }
	print EDK "PARAMETER ", uc($key), "= ";
...
#-----------------------------------------------------------------------------
sub write_ncsim_args
#-----------------------------------------------------------------------------
# could be a problem with ncsim interpreting binary or hex types...
#-----------------------------------------------------------------------------
{
    my ($dir, $core_name, $core_version) = @_;

    open(NCSIM, ">$dir/ncsim_parameters.args") or err('--', "Could not create ncsim_parameters.args for writing:$!", 2);
    ...
    # only write parameters not beginning with C_ (core parameters) to this file
    # all other parameters are set using the parameterise_config.pl script
    if ($key =~ /^C_/i) {
            next;
        }
	print NCSIM "-generic \"", uc($key), "=>";

    my $temp;
	# print out each element of the list, being careful not to add a seperator after the last element
	foreach my $elem (0..$#{$VHT_PARAM{$key}}-1)
	{
        $temp = $VHT_PARAM{$key}[$elem];
        #check to see if in tpr hex format
        if ($temp =~ /^0x/i) {
            $temp =~ s/0x(\w*)/X\\\"$1\\\"/gi;
        } elsif ($temp =~ /^hx/i) {
            $temp =~ s/hx(\w*)/X\\\"$1\\\"/gi;
        # or tpr binary format
        } elsif ($temp =~ /^0b/i) {
            $temp =~ s/0b(\w*)/B\\\"$1\\\"/gi;
        }   else {
            $temp =~ s/\"//g;
    ...
    #set to last element
    $temp = $VHT_PARAM{$key}[$#{$VHT_PARAM{$key}}];
...
sub write_vsim_args
#-----------------------------------------------------------------------------
# could be a problem with vsim interpreting binary or hex types...
#-----------------------------------------------------------------------------
{
    my ($dir, $core_name, $core_version) = @_;

    open(VSIM, ">$dir/vsim_parameters.args") or err('--', "Could not create vsim_parameters.args for writing:$!", 2);
...
#-----------------------------------------------------------------------------
{
    my ($dir, $core_name, $core_version) = @_;
    my @m4defaults;
    #if a top_generic_defs.m4 file already exists then it contains the default values of parameters, read these in
    if (-e $TPRFILEDIR."/top_generic_defs.m4") {
        open(M4DEFAULTS,"<".$TPRFILEDIR."/top_generic_defs.m4") or err('--',"Could not open $TPRFILEDIR\/top_generic_defs.m4 for reading!",2);
        @m4defaults = <M4DEFAULTS>;
        close M4DEFAULTS;
    }

    open(M4GENERICS, ">$dir/top_generic_defs.m4") or err('--', "Could not create top_generic_defs.m4 for writing", 2);
    ...
    foreach $key (keys %VHT_PARAM) {
        ...
        #want the format of the output to be like: ifdef(`C_DCR_DWIDTH',,`define(`C_DCR_DWIDTH',`32')')dnl
	    print M4GENERICS "ifdef\(\`".uc($key)."\'\,\,\`define\(\`".uc($key)."\'\,\`";
        ...
        my $hex_num;
	    foreach my $elem (0..$#{$VHT_PARAM{$key}}-1) {
            if ($VHT_PARAM{$key}[$elem] =~ /^(0b[01]*)/i) {
                $binary_num = $1;
                $decimal_num = oct($1);
                $hex_num = sprintf("0x%x",$decimal_num);
                #print M4GENERICS "$VHT_PARAM{$key}[$elem]\'\)\'\)dnl\n";
	            print M4GENERICS "$hex_num\'\)\'\)dnl\n";
            } else {
                print M4GENERICS "$VHT_PARAM{$key}[$elem]\'\)\'\)dnl\n";
            }
	    }
	    ...
	    # the last line has a \n instead of seperator
        if ($VHT_PARAM{$key}[$#{$VHT_PARAM{$key}}] =~ /^(0b[01]*)/i) {
            $binary_num = $1;
            $decimal_num = oct($1);
            $hex_num = sprintf("0x%x",$decimal_num);
            #print M4GENERICS "$VHT_PARAM{$key}[$elem]\'\)\'\)dnl\n";
            ...
            print M4GENERICS "$VHT_PARAM{$key}[$#{$VHT_PARAM{$key}}]\'\)\'\)dnl\n";
        }
    }

    #finally write the default values to this file, if any of the parameters have not been set they will be set to default value
    print M4GENERICS "\n";
    print M4GENERICS "-- Default Generics defined by testbench developer\n";
    print M4GENERICS "-- These are overridden by the parameters above which are set within the TPR\n";
    print M4GENERICS "\n";
    print M4GENERICS @m4defaults;
...
#-----------------------------------------------------------------------------
{
    ...
    #-----------------------------------------------------------
    # read values from TPR and write to edk array in mhs format
    #-----------------------------------------------------------
    my @edk;
    my $key; 
    push @edk,"BEGIN $core_name\n";
    push @edk, "PARAMETER HW_VER = $core_version\n";
    my @temp_array;
    my $temp_edk;
    foreach $key (keys %VHT_PARAM) {
	    # check to see if the key has a value.  It is possible to have
        ...
        #check to see if parameter begins with port_ if so then this is a port definition
        if ($key =~ /^\s*PORT_/i) {
            $temp = $key;
            $temp =~ s/^\s*PORT_//gi;
            push @temp_array, "PORT ", uc($temp), "= ";
        } else {
	        push @temp_array, "PARAMETER ", uc($key), "= ";
        }

	    # print out each element of the list, being careful not to add a seperator after the last element
	    foreach my $elem (0..$#{$VHT_PARAM{$key}}-1) {
	        push @temp_array, "$VHT_PARAM{$key}[$elem]$seperator";
	    }

	    # the last line has a \n instead of seperator
	    push @temp_array, "$VHT_PARAM{$key}[$#{$VHT_PARAM{$key}}]\n";
        $temp_edk = join " ",@temp_array;
        @temp_array = "";
        push @edk, $temp_edk;
    }
    ...
    #---------------------------------------------------
    # now write all this out to a new system_in.mhs file
    #---------------------------------------------------
    print EDK @edk;
    close EDK;
...
sub write_filerev
# Write out gentest version info, for reference
{
   my $datadir = shift ;
   my $fp = $datadir . "/filerev.txt" ;
   open( FP, ">${fp}" ) or die "Unable to write, $fp\n" ;
   print( FP "\n\nbin = $main::bpath\n" ) ;
   for my $kfile ( 'gentests.pl', 'gentests.pm' ) {
     print( FP $kfile, ",  ", $main::filerev{$kfile}, "\n" ) ;
   }
   close FP ;
}
...
sub write_coe_file
{
     ...
     #print "lbdbg - dir = $dir\nlbdbg - filename = $filename\n" ;

     # defs for cr 433003 fix
     my @pad ;  # Padding strings -to ensure const-width o/p fields
     my @len ;  # Unpadded field widths
     my $maxlen = 0 ; # Width of widest field ..shorter fields will be padded to match
     my $enablePad = 0 ;

     open COE, ">$filename" or err ('--', "Could not create coe file $filename: $!", 2);

     #print "\ninfo(write_coe_file) : Writing coe file $filename\n" ;

     foreach my $param (@paramaters)
     {
          #print "lbdbg - param = $param\n" ;
	  if ($param !~ /^A_GENTESTS_NULL_PARAM/)
	  {
	       ...
	       $write_param =~ s/^.*_FH_//;
               ...
               if( $write_param =~ /^\s*(radix|memory_initialization_radix)\s*$/ ) {
                  my $crad = $VHT_PARAM{$param}[0] ;
                  $enablePad = 1 if $crad == 2 || $crad == 16 ;
                  #print "info(write_coe_file) : coefile radix = $crad, enablePad = $enablePad\n" ;
               }

               # Determine any padding required to ensure each coeff will occupy
               # a const width field in output coe file (cr 433003)
	       foreach my $elem (0..$#{$VHT_PARAM{$param}})  {
		  my $coeno = sprintf( $VHT_PARAM{$param}[$elem] ) ;
                  $len[$elem] = length( $coeno ) ;
                  $maxlen = $len[$elem] if $len[$elem] > $maxlen ;
                  #print "info(write_coe_file) : coeno=$coeno, len =", $len[$elem], "\n" ;
	       }
               PADGEN:
	       foreach my $elem (0..$#{$VHT_PARAM{$param}})  {
                   $pad[$elem] = '', next PADGEN unless $enablePad ;
                   $pad[$elem] = '0' x ($maxlen - $len[$elem]) ;
                   #print "info(write_coe_file) : pad=", $pad[$elem], ", maxlen=$maxlen\n" ;
                   #print "info(write_coe_file) : Applying data padding to coeff, ",
                   # "$VHT_PARAM{$param}[$elem]\n" if length($pad[$elem]) > 0 ;
	       }
	       ...
	       foreach my $elem (0..$#{$VHT_PARAM{$param}}-1)
	       {
                    ...
                    # Dont print any "0." values -print "0" instead (CR448128)
                    my $prtDat = $VHT_PARAM{$param}[$elem] ;
                    $prtDat = "0" if sprintf($prtDat) =~ /^\s*-*0\.\s*$/ ; 
		    print COE $pad[$elem], "$prtDat ";

                    # debug print
                    #print( "info(write_coe_file) : ceff value = '", $pad[$elem], 
                    #"$VHT_PARAM{$param}[$elem]' ($prtDat)\n" ) ;
	       }

	       # the last element has a ;\n instead of space seperator
	       print COE $pad[$#pad], "$VHT_PARAM{$param}[$#{$VHT_PARAM{$param}}];\n";
...
#-----------------------------------------------------------------------------
{
    ...
    if( $IPFLOW eq "dspIP"  and $GLOBAL_FLOWOPTS{'REGENERATED'} == 1  ) {
        #print "lbdbg(write_export_file) : dir = $dir\n" ;
        #print "lbdbg(write_export_file) : filename = $filename\n" ;
        $filename =~ /([^\\\/]+)\s*$/, my $bn = $1 ;
        #print "lbdbg(write_export_file) : bn = $bn\n" ;
        if( -f $bn ) {
           copy("./$bn", "$filename") or err('--', "Could not create $filename", 2);
           return ;
         }
    }
...
#-----------------------------------------------------------------------------
{
    ...
    {
	$cfg_exists = 1;
	%settings = read_cfg($TPRFILEDIR."/gentests.cfg");
...
#-----------------------------------------------------------------------------
{
    ...
    {
	my %settings = read_cfg($TPRFILEDIR."/gentests.cfg");
	...
	if (exists $settings{'ARRAY'}{'VHD_TBs'})
	{
		...
		# use the _orig tb if it exists
 		if (-e $file."_orig")
 		{
		    File::Copy::copy($file."_orig", $COMMON_DATA_DIR."/".$file) or err('--', "Could not copy the $file to $COMMON_DATA_DIR", 2);
 		}
	    }
	}
	...
	if ($file !~ /\$/)
	{
	    File::Copy::copy($file, $COMMON_DATA_DIR) 
             or err('--', "Could not copy the $file to $COMMON_DATA_DIR", 2);
	}
    }
...
#-----------------------------------------------------------------------------
{
    ...
    # added delete of common files area: niamhc 23/06/2006
    if (-e $COMMON_DATA_DIR) {
	    rmtree([$COMMON_DATA_DIR]) or err('--', "Could not delete directory $COMMON_DATA_DIR", 10);
    }
    if (!-e $COMMON_DATA_DIR)
    {
	mkpath([$COMMON_DATA_DIR]) or err('--', "Cannot create the dir $COMMON_DATA_DIR: $!", 2);
...
#-----------------------------------------------------------------------------
{
   ...
   if( $rhs =~ /^\$?\w+(,\$?\w+)*$/i ) {
      $pefix = "" ;  $vals  = $rhs ;  $pofix = "" ; $setpiece = 1 ;
   }
   
   # Extract value list in RAND call
   elsif( $rhs =~ /^(\[rand:range:)(\$?\w+(,\$?\w+)*)(\W.*)$/i ) {
      $pefix = $1 ;  $vals  = $2 ;    $pofix = $4 ;  $setpiece = 1 ;
   }
   ...
   # rhs format is not one of the predefined formats catered for above ..just
   # go thru it + lc any var refs (dollar followed by alpha-numerics)
   $rhs =~ s/(\$)([a-zA-Z]\w*)/$1 . lc($2)/eg ;
   return $rhs ;
...
#-----------------------------------------------------------------------------
{
    return $flow_data{$whattodo}{'ipflow'};
}

#-----------------------------------------------------------------------------

sub modify_final_cfg
# New subroutine for Arizona.
# If generating regression tests, Final.cfg will be copied to new (regression) testcase dir from results area.
# However, it will contain parameter settings which are incorrect for regression tests.
# Some need to be modified, others can be removed altogether.
# Some options are correct for XIR but incorrect for XIN. As these options do not change from testcsase to testcase, they can also be removed.
# Example: respository_win, respository_unix, etc. need to be changed.
# Example: CADSET_xilinx needs to be removed
# Example: DISPLAY matisse:0, XILINXD_LICENSE_FILE 1800@efpga, need to removed to prevent XIN problems.
# Stephen Breslin, 7 Feb 2006.
#-----------------------------------------------------------------------------
{
   my $cfg_file = $_[0].$_[1];            # [0] is datadir path, [1] is file name.
   my $datadir = $_[0];
   my $repos = $main::REPOSITORY;         # Passed into gentests.pl by regression_tests.pl
   my $regression_testlevel = 3;          # 3 for regressions. Used by run script: netIP2.pl
   my $regressions_vsim_timelimit = "10";
   my $regressions_runlimit_fail = "0";
   my %settings;
   my $skip = 0;
   
   open CFG, $cfg_file or die "ERROR: could not read cfg file $cfg_file: $!\n";

   $main::DEBUG && print "gentests::modify_final_cfg: cfg_file = $cfg_file\n";
   $main::DEBUG && print "gentests::modify_final_cfg: datadir = $datadir\n";
   $main::DEBUG && print "gentests::modify_final_cfg: repos = $repos\n";   
   $main::DEBUG && print "gentests::modify_final_cfg: regression_testlevel = $regression_testlevel\n";
   $main::DEBUG && print "\ngentests::modify_final_cfg: Modifying Final.cfg options...\n";
	...
	#$_ =~ m/(\S+)\s+(\S+)\s*$/;
	$_ =~ m/(\S+)\s+(\S)(.*)$/;            # Will also match param. settings such as "COREGEN_OPTION -J DSV_TEST=1"
	my $key = $1;
	my $value = $2.$3;
	

        # Overwrite values for certain keys or delete keys & value.
	# For example, CADSET_xilinx will need to be deleted here as run script will take care of it.
	if ($key eq "REPOSITORY_WIN") 
	{
	   $value = $repos;                    # $repos is UNIX version, need to modify.
	   $value =~ s/\/proj/G:/;             # Sub /proj with G:
	   $value =~ s/\//\\/g;                # Sub slashes with backslashes
	   $main::DEBUG && print "Modified REPOSITORY_WIN to $value\n"; 
	}
	elsif ($key eq "REPOSITORY_UNIX")
	{
	   $value = $repos;
	   $main::DEBUG && print "Modified REPOSITORY_UNIX to $value\n";
	}
	elsif ($key eq "TESTLEVEL")
	{
	   $value = $regression_testlevel;
	   $main::DEBUG && print "Modified TESTLEVEL to $value\n";
	}
	elsif ($key eq "VSIM_TIMELIMIT")
	{
	   $value = $regressions_vsim_timelimit;
	   $main::DEBUG && print "Modified VSIM_TIMELIMIT to $value minutes.\n";
	}	
	elsif ($key eq "RUNLIMIT_FAIL")        # Required if VSIM_TIMELIMIT changed to a short time to prevent failures due to timeout
	{
	   $value = $regressions_runlimit_fail;
	   $main::DEBUG && print "Modified RUNLIMIT_FAIL to $value.\n";
	}	

        # Don't want any of the following params written to the new Final.cfg.
	# They are site or platform-dependent paths.
	# Let the run script or easytest decide what platform to run on,
	# or let the site/flow-specific cfg files set these options.
	# Also, CADSET_xilinx will be defined by the run flow at runtime.
	elsif ( ($key eq "CADSET_xilinx") or
	        ($key eq "CADSET_edk") or
	        ($key eq "perl") or
	        ($key eq "REPOSITORY") or
	        ($key eq "REPOSITORYROOT") or
	        ($key eq "RESULTSROOT") or
	        ($key eq "RESULTSROOT_UNIX") or
	        ($key eq "SPOOLER") or
	        ($key eq "XCOMPLIBS") or
	        ($key eq "tc") or
	        ($key eq "DISPLAY") or
	        ($key eq "XILINXD_LICENSE_FILE") or
	        ($key eq "SITE") or
	        ($key eq "CADGROUP") or
	        ($key eq "HOME") )
	{
	   $skip = 1;
	   $main::DEBUG && print "Not writing $key to new Final.cfg\n";
	}	
   ...
   my $temp = $datadir . "/temp";
   $main::DEBUG && print "gentests::modify_final_cfg: temp = $temp\n";
   $main::DEBUG && print "\ngentests::modify_final_cfg: Listing full contents of new Final.cfg...\n";
   open TEMP, ">$temp" or die "ERROR: could not create temp file: $!\n";
   
   foreach my $key (keys %settings)
   {
      $main::DEBUG && print "gentests::modify_final_cfg: $key $settings{$key}\n";
      print TEMP "$key $settings{$key}\n";
   }
   close TEMP;

   my $command = "mv $cfg_file $cfg_file" . ".original";
   $main::DEBUG && print "\ngentests::modify_final_cfg: command = $command\n";
   if (system("$command") == 1) {die "Could not rename $cfg_file\n";}
   if (system("mv $temp $cfg_file") == 1) {die "Could not rename temp to $cfg_file\n";}
