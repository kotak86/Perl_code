...
my %flow_data = (
		 'simulate' => {
				...
				'mti' => ["IPtest_mti_setup.flw", "IPtest_mti_sim.flw"],
				# the order in which the MXE files are run
				# the first file must be the setup file
				'mxe' => ["IPtest_mxe_setup.flw", "IPtest_mti_sim.flw"],
				# the order in which the NCSIM files are run
				# the first file must be the setup file
				'ncsim' => ["IPtest_ncsim_setup.flw", "IPtest_ncsim_sim.flw"],
				# the order in which the SCIROCCO/VCS files are run
				# the first file must be the setup file
				'scirocco' => ["IPtest_scirocco_setup.flw", "IPtest_scirocco_sim.flw"],
				'vcs' => ["IPtest_scirocco_setup.flw", "IPtest_scirocco_sim.flw"],
			       },
		 ...
		 'structiptest' => {
				    'ipflow' => "structIPtest",
				    'main' => "structIPtest.flw",
				    'simulators' => ["mti"],
				    'mti' => ["structIPtest_mti_setup.flw", "structIPtest_mti_sim.flw"],
				   },
		 ...
		 'characterize' => {
				    'ipflow' => "IPcharacterise",
				    'main' => "characterize.flw",
				   },
		);
...
#-----------------------------------------------------------------------------
{
    ...
    if (-e "$tpr_dir/create_testcases_randomonly.pl") {
        ...
        if (system("chmod 777 testcasedata.dat") != 0) {
            die "Couldn't make testcasedata.dat editable\n";
        }
...
sub write_test
{
    ...
    elsif (($IPFLOW =~ /^edkipsys$/i) || ($IPFLOW =~ /edk_dev/i))
    {
...
#-----------------------------------------------------------------------------
{
    ...
    open(EDK, ">$dir/system_in.mhs") or err('--', "Could not create system_in.mhs for writing:$!", 2);


    #-----------------------------------------------------------
    # check COPYFILES to see if there's a system_in.mhs
    # if so read it in and remove it from the local copyfiles list
    #-----------------------------------------------------------
    my $i;
    my @system;
    my $file;
    for ($i = 0; $i <= $#COPYFILES; $i++) {
        $file = $COPYFILES[$i];
        if ($file =~ /system_in.mhs/) {
            if ($^O =~ /^MSWin/) {
	            $file = $TPRFILEDIR."\\$file";
	            $file =~ s/\//\\/g;
	        } else {
            ...
            # remove system_in.mhs from COPYFILES list
            splice @COPYFILES, $i, 1;
            # open system_in.mhs and read in the data, will be used later
            open MHS, $file;
            @system = <MHS>;
            close MHS;
            # should be only one system_in.mhs file!
            last;
        }
    }
    #-----------------------------------------------------------
    ...
    #------------------------------------------------------------------------------------------------------------
    # now put the @system and @edk lists together, @edk overwrite @system if they are defining the same parameter
    #------------------------------------------------------------------------------------------------------------
    my $found_core = 0;
    my $parameter_edk;
    my $parameter_system;
    ...
    for ($j = 0; $j <= $#system; $j++) {
        if ($found_core == 1) {
            for ($i = 0; $i <= $#edk; $i++) {
                # find the name of the port or parameter in the system_in.mhs file
                if (($system[$j] =~ /PARAMETER\s*(\w+)\s*=/) || ($system[$j] =~ /PORT\s*(\w+)\s*=/)) {
                    $parameter_system = $1;
                    # if this port or parameter exists in the TPR file
                    # find the parameter in the edk line
                    if (($edk[$i] =~ /PARAMETER\s*(\w+)\s*=/)||($edk[$i] =~ /PORT\s*(\w+)\s*=/)) {
                        $parameter_edk = $1;
                    }
                    if ($parameter_edk eq $parameter_system) {
                        # overwrite it
                        $system[$j] = $edk[$i];
                        # and remove from the rest of the edk list of parameters
                        splice @edk,$i,1;
                        # reset parameter_edk
                        $parameter_edk = "";
                    }
                } elsif ($system[$j] =~ /END/) {
                    # write the rest of the edk parameters which weren't in the system_in.mhs file before the END
                    print "System is:\n @system\n";
                    print "$system[$j] replaced with @edk\n";
                    splice @system, $j, 1, @edk;
                    print "System is:\n @system\n";
                    $found_core = 0;
                }
            }
        }
        if ($system[$j] =~ /BEGIN\s+$core_name/) {
            $found_core = 1;
            # get rid of the begin at the start of the edk array
            if ($edk[0] =~ /BEGIN\s+$core_name/) {
                shift @edk;    
            }
        } 
    }
