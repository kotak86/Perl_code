...
use XML::Simple;
use Data::Dumper;
my $file_parse = $ARGV[0];
my @splt_file = split/\./, $file_parse; my $file = $splt_file[0];
...
my $family = "FAMILY";  my $core = $dsp_cores{$IP};
my $cmp; my $name; my @nme; my $cont;
my $tcase = "$file.xco_new";
open XCO, ">>$tcase" or die $!;
...
for (my $i=0; $i<200; $i++){
	...
	if ($name =~ m/PARAM_VALUE|PROJECT_PARAM/){
